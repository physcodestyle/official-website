...
### Команды для сборки и сервисной поддержки

1. Работа с репозиторием с поддержкой перезапуска браузера после сборки:

    ```bash
    $ npm run start
    ```

2. Сборка сайта для заливки на сервер:

    ```bash
    $ npm run build
    ```

3. Проверка соответствия правилам форматирования кода всего репозитория:

    ```bash
    $ npm run editorconfig
    ```

4. Проверка соответствия правилам форматирования файлов в формате Markdown:

    ```bash
    $ npm run markdown
    ```

5. Проверка соответствия кода HTML страниц собранного сайта стандартам W3C (обязательна поддержка Java):

    ```bash
    $ npm run html
    ```

6. Проверка соответствия правилам форматирования файлов стилей:

    ```bash
    $ npm run css
    ```

7. Заливка карт для англоязычной страницы контактов:

    ```bash
    $ npx gulp map
    ```

8. Автоматический сбор последних новостей на сайт:

    ```bash
    $ npx gulp watch
    ```

9. Запуск в режиме отслеживания изменения стилей и скриптов на сайте:
...
Для формирования названий классов применяется технология [БЭМ](https://ru.bem.info). Исользуется европейская нотация: для обозначения элемента используется `__`, а для обозначения модификатора используется `--`.

При разработке сайта применяется технология Mobile-first. Сначала создается мобильная версия блока, а потом дизайн блока для больших размеров с помощью @media. Нет брекпоинтов для адаптации. Каждый блок сам определяет для себя, когда ему пора меняться.
